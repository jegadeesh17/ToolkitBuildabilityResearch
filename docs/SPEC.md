...
- **Recorded deviation (2026-09-24):** hand-labelling all 140 fields did not fit the time available. The 20-app sample was instead labelled by an independent AI agent (Gemini CLI, a model family not used anywhere in the pipeline) working blind in a folder outside the repository, with a verbatim quote and URL per field; a person then spot-checked one reference verdict per category (`verification/spot_check.html`). Scores are therefore reported as **agreement with reference labels**, not human-verified accuracy, and the page says so.
